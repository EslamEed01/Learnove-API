...
{
    public class AuthService(UserManager<AppUser> userManager, IJwtProvider jwtProvider, ILogger<AuthService> logger
    , IHttpContextAccessor httpContextAccessor, IEmailSender emailSender, LearnoveDbContext context, IMapper mapper, SignInManager<AppUser> signInManager) : IAuthService
    {
        ...
        private async Task SendConfirmationEmail(AppUser user, string code)
        {
            ...
            BackgroundJob.Enqueue(() => _emailSender.SendEmailAsync(user.Email!, "✅ Learnove : Email Confirmation", emailBody));
        ...
        public async Task SendResetPasswordEmail(AppUser user, string code)
        {
            ...
            BackgroundJob.Enqueue(() => _emailSender.SendEmailAsync(user.Email!, "✅Learnove: Change Password", emailBody));
        ...
        Task IAuthService.SendConfirmationEmail(AppUser user, string code)
        {
            return SendConfirmationEmail(user, code);
        }


        Task<(IEnumerable<string> roles, IEnumerable<string> permissions)> IAuthService.GetUserRolesAndPermissions(AppUser user, CancellationToken cancellationToken)
        {
